...
import { IAuthority } from '../../../core/models/interfaces/IAuthority';
...
export class LvAuthorirySetupComponent implements OnInit {
  ...
  currentSection: string = 'employee';
  ngOnInit(): void {
  ...
  loadData<T>(
    serviceMethod: (pageIndex: number, pageSize: number, filter: string, sortColumn: string, sortDirection: string) => Observable<PaginatedResponse<T>>,
    callback: (data: T[], total: number) => void
  ): void {
    this.loading = true;
  
    serviceMethod(this.pageIndex, this.pageSize, this.filter, this.sortColumn, this.sortDirection)
      .subscribe({
        next: (response: PaginatedResponse<T>) => {
          callback(response.content, response.totalElements);
          this.loading = false;
        },
        error: () => {
          this.loading = false;
  ...
  loadEmployees(): void {
    this.loadData<IEmployee>(
      (pageIndex, pageSize, filter, sortColumn, sortDirection) => 
        this.leaveService.GetEmployees(pageIndex, pageSize, filter, sortColumn, sortDirection, this.selectedDepartmentIds),
      (data, total) => {
    ...
    console.log('Data from loadEmployees', this.selectedDepartmentIds);
  }
 ...
 // Default section
  ...
  loadAuthority(): void {
    ...
    this.loadData<IAuthority>(
      (pageIndex, pageSize, filter, sortColumn, sortDirection) => 
        this.leaveService.GetAuthority(pageIndex, pageSize, filter, sortColumn, sortDirection),
      (data, total) => {
        this.lvAuthoritydataSource = data;
        this.totalData = total;
      }
    );
  }
